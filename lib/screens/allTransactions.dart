  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < allTransactions.length; i++) {
      if (allTransactions[i]['isIncome'] == false) {
        expenseTransactions.add([transactionKeys[i], allTransactions[i]]);
      } else {
        incomeTransactions.add([transactionKeys[i], allTransactions[i]]);
      }
    }
    List options = [];
    if (!SortData.loading) {
                      ...
                      Row(
                          ...
                          children: [
                            FilterChip(
                              ...
                              selected: SortData.isDisplayIncome,
                              onSelected: (bool selected) {
                                setState(() {
                                  displayTransactions = options[SortData.index];
                            ...
                            FilterChip(
                              ...
                              selected: SortData.isDisplayExpense,
                              onSelected: (bool selected) {
                                setState(() {
                                  displayTransactions = options[SortData.index];
                    ...
                    ? Row(
                        ...
                        children: [
                            DropdownButton(
                              ...
                              dropdownColor:
                                  const Color(0xFF4A5859).withOpacity(0.9),
                              items: SortData.yearMenu.map((element) {
                                return DropdownMenuItem(
                                    value: SortData.yearMenu.indexOf(element),
                                    child: Text(element));
                              }).toList(),
                              onChanged: (value) {
                                setState(() {
                                  ...
                                  value == 0
                                      ...
                                      : {
                                          ...
                                          setState(() {
                                            SortData.yearHint =
                                                SortData.yearMenu[value as int];

                                            SortData.currentYear =
                                                SortData.yearMenu[value];
                                            SortData.index = 2;
                                            monthMenu = SortData.sortYear()[1];
                                          }),
                                        };
                                });
                              },
                            ...
                            isYearSelected
                                ? DropdownButton(
                                    borderRadius: BorderRadius.circular(10),
                                    dropdownColor: const Color(0xFF4A5859)
                                        .withOpacity(0.9),
                                    hint: Text(SortData.monthHint),
                                    items: monthMenu.map((element) {
                                      return DropdownMenuItem(
                                          value: element[1],
                            ...
                            isMonthSelected && isYearSelected
                                ? DropdownButton(
                                    ...
                                    dropdownColor: const Color(0xFF4A5859)
                                        .withOpacity(0.9),
                                    hint: Text(SortData.dayHint),
                                    items: dayMenu.map((element) {
                                      return DropdownMenuItem(
                                          value: element[1],
                                          child: Text(element[0]));
                                    }).toList(),
                        ...
                        child: ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              return TransactionTile(
                                transactionName: displayTransactions[index][1]
                                    ['Title'],
                                money: displayTransactions[index][1]['Amount'],
                                dateTime: DateTime.fromMillisecondsSinceEpoch(
                                    displayTransactions[index][1]['Date']
                                        .millisecondsSinceEpoch),
                                isIncome: displayTransactions[index][1]
                                    ['isIncome'],
                                transactionId: displayTransactions[index][0],
                                category: displayTransactions[index][1]
                                    ['Category'],
                              );
                            }),
                      ))
